...
export function createServerInstance(version: string) {
  const server = new McpServer(
    { name: "OneContext", version },
    {
      instructions:
        "Use this server to manage long-horizon agent context with Git-like operations and share Codex session segments as resources.",
    }
  );
  ...
  server.registerResource(
    "onecontext-gcc-branch-commit",
    new ResourceTemplate("onecontext://gcc/branch/{branch}/commit", { list: undefined }),
    {
  ...
  server.registerResource(
    "onecontext-gcc-branch-commits",
    new ResourceTemplate("onecontext://gcc/branch/{branch}/commits", { list: undefined }),
    {
  ...
  server.registerResource(
    "onecontext-gcc-branch-log",
    new ResourceTemplate("onecontext://gcc/branch/{branch}/log/{tail}", { list: undefined }),
    {
  ...
  server.registerResource(
    "onecontext-gcc-branch-metadata",
    new ResourceTemplate("onecontext://gcc/branch/{branch}/metadata", { list: undefined }),
    {
  ...
  // Segment resources
  server.registerResource(
    "onecontext-segment-meta",
    new ResourceTemplate("onecontext://segment/{id}/meta", { list: undefined }),
    {
    ...
    async (uri, { id }) => {
      const seg = String(id);
      if (!isSafeName(seg)) throw new Error(`Invalid segment id: ${seg}`);
      return {
  ...
  server.registerResource(
    "onecontext-segment-transcript",
    new ResourceTemplate("onecontext://segment/{id}/transcript", { list: undefined }),
    {
  ...
  server.registerResource(
    "onecontext-segment-events",
    new ResourceTemplate("onecontext://segment/{id}/events", { list: undefined }),
    {
  ...
  server.registerResource(
    "onecontext-segment-blob",
    new ResourceTemplate("onecontext://segment/{id}/blob/{sha256}", { list: undefined }),
    {
      title: "Segment blob",
    ...
    async (uri, { id, sha256 }) => {
      ...
      if (!isSafeName(seg)) throw new Error(`Invalid segment id: ${seg}`);
      if (!/^[a-f0-9]{64}$/.test(h)) throw new Error(`Invalid sha256: ${h}`);
      return {
  ...
  server.registerTool(
    ...
    async ({ name, purpose, checkout, overwrite }) => {
      try {
        if (!isSafeName(name)) throw new Error(`Invalid branch: ${name}`);
        ...
        return {
          content: [
            {
              type: "text",
              text: `Branch ${name} ${created ? "created" : "ready"} at ${branchDir}${
  ...
  server.registerTool(
    ...
    async ({ summary, details, branch, maxInlineChars }) => {
      try {
        ...
        if (b && !isSafeName(b)) throw new Error(`Invalid branch: ${b}`);
  ...
  server.registerTool(
    ...
    async ({ fromBranch, intoBranch, summary, includeLogTail, maxInlineChars, updateMain }) => {
      try {
        if (!isSafeName(fromBranch)) throw new Error(`Invalid fromBranch: ${fromBranch}`);
        if (!isSafeName(intoBranch)) throw new Error(`Invalid intoBranch: ${intoBranch}`);
  ...
  server.registerTool(
    "gcc-export",
    {
      title: "GCC Export",
      description:
        "Export a bounded commit range into a shareable segment (transcript + events). Use lastN or fromCommitId/toCommitId.",
      inputSchema: {
        branch: z.string().optional().describe("Branch name (defaults to current)"),
    ...
    async ({
      ...
      try {
        ...
        if (fromCommitId && !isSafeName(fromCommitId)) throw new Error(`Invalid fromCommitId: ${fromCommitId}`);
        if (toCommitId && !isSafeName(toCommitId)) throw new Error(`Invalid toCommitId: ${toCommitId}`);
  ...
  server.registerTool(
    "forward-codex-session-segment",
    {
      ...
      description:
        "Export a slice of a Codex session *.jsonl file into a structured segment (events + transcript).",
      inputSchema: {
    ...
    async ({ sessionPath, startLine, endLine, mode, maxInlineChars, storeImages, attachToBranch }) => {
      try {
        ...
        const branchExists =
          attachToBranch && isSafeName(attachToBranch)
