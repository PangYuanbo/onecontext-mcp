...
export function readCommitRecords(cwd: string, branch: string): CommitRecord[] {
  ...
  for (const line of lines) {
    try {
      ...
      const rec = parsed as CommitRecord;
      if (!rec.id || !rec.ts || !rec.summary || !rec.branch) continue;
      records.push(rec);
    } catch {
      // ignore bad lines
    }
...
export function searchCommitRecords(
  cwd: string,
  branch: string,
  query: string,
  limit: number
  ...
  const scored: { rec: CommitRecord; score: number }[] = [];
...
export function selectCommitRange(opts: {
...
}): { selected: CommitRecord[]; fromId: string; toId: string } {
  ...
  if (fromIndex === -1) throw new Error(`fromCommitId not found: ${fromId}`);
  if (toIndex === -1) throw new Error(`toCommitId not found: ${toId}`);
