...
function formatCommitTranscript(rec: CommitRecord): string {
  const parts: string[] = [];
...
export async function exportCommitRange(opts: ExportCommitsOptions): Promise<{
  segmentId: string;
  meta: SegmentMeta;
}> {
  const {
  ...
  for (const rec of selected) {
    if (rec.kind === "merge") merges += 1;
    store.writeEventLine(
      JSON.stringify({
        ts: rec.ts,
        kind: "meta",
  ...
  let statBlobSha256: string | undefined;
  let patchBlobSha256: string | undefined;
