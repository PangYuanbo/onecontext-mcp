import { commitMilestone, CommitRecord } from "./commit.js";
...
export function mergeBranch(opts: MergeOptions): CommitRecord {
  const {
    cwd,
    fromBranch,
    intoBranch,
  ...
  if (!pathExists(fromDir)) throw new Error(`Branch not found: ${fromBranch}`);
  if (!pathExists(intoDir)) throw new Error(`Branch not found: ${intoBranch}`);
  ...
  appendText(
    intoLog,
    `${JSON.stringify({ ts: nowIso(), kind: "merge", fromBranch, intoBranch, commitId: record.id })}\n`
  );
