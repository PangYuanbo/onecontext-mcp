...
import { SegmentEvent, SegmentMeta, SegmentMode } from "../segments/types.js";
...
function isIgnorableImageWrapper(text: string): boolean {
  const t = text.trim();
  return t === "<image>" || t === "</image>";
}

function writeMessage(store: ReturnType<typeof createSegmentStore>, ev: SegmentEvent): void {
...
function writeTool(store: ReturnType<typeof createSegmentStore>, ev: SegmentEvent): void {
  store.writeEventLine(JSON.stringify(ev));
...
function writeMeta(store: ReturnType<typeof createSegmentStore>, ev: SegmentEvent): void {
  store.writeEventLine(JSON.stringify(ev));
}

export async function forwardCodexSessionSegment(opts: ForwardCodexOptions): Promise<{
  segmentId: string;
  meta: SegmentMeta;
}> {
  const {
  ...
  for await (const line of rl) {
    ...
    if (payloadType === "message") {
      ...
      const role: SegmentEvent["role"] =
        roleRaw === "user" || roleRaw === "assistant" || roleRaw === "developer" ? roleRaw : undefined;
      ...
      for (const part of content) {
        ...
        if (partType === "input_text" || partType === "output_text") {
          ...
          if (isIgnorableImageWrapper(text)) continue;
    ...
    if (payloadType === "function_call_output" || payloadType === "custom_tool_call_output") {
      ...
      const inline =
        typeof rawOutput === "string" ? store.maybeStoreLargeText("tool_output", rawOutput).inline : rawOutput;
    ...
    if (payloadType === "ghost_snapshot") {
      writeMeta(store, { ts, kind: "meta", meta: { ghostSnapshot: (p.ghost_commit as unknown) || null } });
      stats.events += 1;
      continue;
    }
  }
